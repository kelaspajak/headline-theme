...
function renderRelatedPosts(posts, headingText, showExcerpt) {
    ...
    const postsHTML = posts.map(post => {
        ...
        return `
            <a class="resource-list-item" href="${post.url}">
                <span class="resource-list-item-title">${safeTitle}</span>
                <div class="resource-list-item-meta"><span>${readingTime} min read<svg viewBox="0 0 18 27" xmlns="http://www.w3.org/2000/svg"><path d="M2.397 25.426l13.143-11.5-13.143-11.5" stroke-width="3" stroke="currentColor" fill="none" fill-rule="evenodd" stroke-linecap="round" stroke-linejoin="round"/></svg></span></div>
            </a>
        `;
    }).join('');
